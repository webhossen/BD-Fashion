...
document.addEventListener("DOMContentLoaded", () => {
    ...
    const orderNowLink = document.getElementById("designBtn");
    ...
    function updateOrderNowLink() {
        ...
        orderNowLink.href = `order.html?productId=${product.id}&size=${encodeURIComponent(size)}&qty=${qty}`;
    }

    sizeInput.addEventListener("change", updateOrderNowLink);
    qtyInput.addEventListener("input", updateOrderNowLink);
    updateOrderNowLink();
...
function renderReviews(productId) {
    ...
    list.innerHTML = reviews
        .map((r, index) => `
            <div class="review-item bg-white p-4 rounded-lg border mb-4">
                <div class="review-stars text-yellow-400 text-lg mb-2">${"★".repeat(r.stars)}${"☆".repeat(5 - r.stars)}</div>

                <div class="flex items-center gap-2 mb-2">
                    <div class="review-name font-semibold text-gray-900">${r.name}</div>
                    ${r.verified ? `<span class="text-xs bg-green-600 text-white px-2 py-0.5 rounded">Verified</span>` : ""}
                </div>

                <div class="review-text text-gray-700 mb-2">${r.text}</div>

                <div class="text-xs text-gray-400">${r.date}</div>

                <button class="review-delete text-red-600 hover:text-red-800 text-sm mt-2 cursor-pointer"
